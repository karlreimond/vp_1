...
function storenewrolerelation($selectedfilm, $selectedperson, $selectedposition, $role){
	$notice = "";
	$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("SELECT person_in_movie_id FROM person_in_movie WHERE movie_id = ? AND person_id = ? AND position_id = ? AND role = ?");
	echo $conn->error;
	$stmt->bind_param("iiis", $selectedfilm, $selectedperson, $selectedposition, $role);
	$stmt->bind_result($idfromdb);
	$stmt->execute();
	if($stmt->fetch()){
		...
		echo $conn->error;
		$stmt->bind_param("iiiis", $personinmovieid, $selectedperson, $selectedfilm, $selectedposition, $role);
		if($stmt->execute()){
